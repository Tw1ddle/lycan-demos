...
class StringTransformDemo extends BaseDemoState {
	private static var strings:Array<Array<String>> = [
...
class TweeningTransformingText extends TransformingText {
	...
	override private function keepLetter(s:String, idx:Int):Void {
		var letter = letters[idx];
		FlxTween.tween(letter, { y: letter.y - 20 }, 1, { ease: EaseSinusoidal.inOutSine, type: FlxTween.PINGPONG, onComplete: function(t:FlxTween):Void {
			if (t.executions == 2) {
				t.cancel();
			}
		}});
	...
	override private function insertLetter(s:String, src:Int, target:Int):Void {
		var letter = getLetter(s);
		letter.alpha = 0;
		letter.y += 100;
		letters.insert(target + 1, letter);
		FlxTween.tween(letter, { y : letter.y - 100, alpha: 1 }, 2, { ease: EaseExponential.outInExpo });
	}

	override private function deleteLetter(s:String, idx:Int):Void {
		var letters = letters.splice(idx, 1);
		var letter = letters[0];
		FlxTween.tween(letter, { y : letter.y - 100, alpha: 0 }, 2, { ease: EaseExponential.outInExpo, onComplete: function(t:FlxTween):Void {
			remove(letter, true);
		}});
	}

	override private function substituteLetter(r:String, i:String, idx:Int):Void {
		var oldLetter = letters[idx];
		FlxTween.tween(oldLetter, { y : oldLetter.y - 100, alpha: 0 }, 2, { ease: EaseExponential.outInExpo, onComplete: function(t:FlxTween):Void {
		...
		FlxTween.tween(newLetter, { y : newLetter.y - 100, alpha: 1 }, 2, { ease: EaseExponential.outInExpo });
	}
